...
def add_flutter(request):
    if request.method == 'POST':
        Donation(user=int(request.POST.get('user')), title=request.POST.get('title'), description=request.POST.get('description'), target=int(request.POST.get('target')), achieved=0, is_ongoing=True).save()
